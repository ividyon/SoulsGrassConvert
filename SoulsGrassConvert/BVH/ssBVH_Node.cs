...
namespace SoulsGrassConvert.BVH
{
    public class ssBVHNode<GO> {
        ...
        public void refit_ObjectChanged(SSBVHNodeAdaptor<GO> nAda, GO obj) {
         ...
         private void expandVolume(SSBVHNodeAdaptor<GO> nAda, Vector3 objectpos, float radius) {
            ...
            if ((objectpos.X - radius) < box.Min.X) {
                box.Min = box.Min with { X = (objectpos.X - radius) }; expanded = true;
            }
            if ((objectpos.X + radius) > box.Max.X) {
                box.Max = box.Max with { X = (objectpos.X + radius) }; expanded = true;
            }
            // test min Y and max Y against the current bounding volume
            if ((objectpos.Y - radius) < box.Min.Y) {
                box.Min = box.Min with { Y = (objectpos.Y - radius) }; expanded = true;
            }
            if ((objectpos.Y + radius) > box.Max.Y) {
                box.Max = box.Max with { Y = (objectpos.Y + radius) }; expanded = true;
            }
            // test min Z and max Z against the current bounding volume
            if ( (objectpos.Z - radius) < box.Min.Z ) {
                box.Min = box.Min with { Z = (objectpos.Z - radius) }; expanded = true;
            }
            if ( (objectpos.Z + radius) > box.Max.Z ) {
                box.Max = box.Max with { Z = (objectpos.Z + radius) }; expanded = true;
            }
        ...
        private void assignVolume(Vector3 objectpos, float radius) {
            box.Min = box.Min with { X = objectpos.X - radius };
            box.Max = box.Max with { X = objectpos.X + radius };
            box.Min = box.Min with { Y = objectpos.Y - radius };
            box.Max = box.Max with { Y = objectpos.Y + radius };
            box.Min = box.Min with { Z = objectpos.Z - radius };
            box.Max = box.Max with { Z = objectpos.Z + radius };
        }
        ...
        internal static float SA(GRASS.BoundingBox box) {
        ...
        internal static float SA(ref GRASS.BoundingBox box) {
            float x_size = box.Max.X - box.Min.X;
            float y_size = box.Max.Y - box.Min.Y;
            float z_size = box.Max.Z - box.Min.Z;
        ...
        internal static BoundingBox AABBofPair(ssBVHNode<GO> nodea, ssBVHNode<GO> nodeb) {
            BoundingBox box = nodea.box;
        ...
        internal float SAofPair(BoundingBox boxa, BoundingBox boxb) {
            BoundingBox pairbox = boxa;
            pairbox.ExpandToFit(boxb);
            return SA(ref pairbox);
        }
        internal static BoundingBox AABBofOBJ(SSBVHNodeAdaptor<GO> nAda, GO obj) {
            float radius = nAda.radius(obj);
            BoundingBox box = new();
            box.Min = box.Min with { X = -radius };
            box.Max = box.Max with { X = radius };
            box.Min = box.Min with { Y = -radius };
            box.Max = box.Max with { Y = radius };
            box.Min = box.Min with { Z = -radius };
            box.Max = box.Max with { Z = radius };
            return box;
        }
        ...
        internal void addObject(SSBVHNodeAdaptor<GO> nAda, GO newOb, ref GRASS.BoundingBox newObBox, float newObSAH) {
        ...
        internal static void addObject(SSBVHNodeAdaptor<GO> nAda, ssBVHNode<GO> curNode, GO newOb, ref GRASS.BoundingBox newObBox, float newObSAH) {
        ...
        internal void findOverlappingLeaves(SSBVHNodeAdaptor<GO> nAda, BoundingBox aabb, List<ssBVHNode<GO>> overlapList) {
            if (toAABB().IntersectsBoundingBox(aabb)) {
                if (gobjects != null) {
                    overlapList.Add(this);
                } else {
        ...
        internal BoundingBox toAABB() {
            ...
            aabb.Min = new Vector3(box.Min.X, box.Min.Y, box.Min.Z);
            aabb.Max = new Vector3(box.Max.X, box.Max.Y, box.Max.Z);
            return aabb;
        }
        ...
        internal void childExpanded(SSBVHNodeAdaptor<GO> nAda, ssBVHNode<GO> child) {
            ...
            if (child.box.Min.X < box.Min.X) {
                box.Min = box.Min with { X = child.box.Min.X }; expanded = true;
            }
            if (child.box.Max.X > box.Max.X) {
                box.Max = box.Max with { X = child.box.Max.X }; expanded = true;
            }
            if (child.box.Min.Y < box.Min.Y) {
                box.Min = box.Min with { Y = child.box.Min.Y }; expanded = true;
            }
            if (child.box.Max.Y > box.Max.Y) {
                box.Max = box.Max with { Y = child.box.Max.Y }; expanded = true;
            }
            if (child.box.Min.Z < box.Min.Z) {
                box.Min = box.Min with { Z = child.box.Min.Z }; expanded = true;
            }
            if (child.box.Max.Z > box.Max.Z) {
                box.Max = box.Max with { Z = child.box.Max.Z }; expanded = true;
            }
        ...
        internal static void childRefit(SSBVHNodeAdaptor<GO> nAda, ssBVHNode<GO> curNode, bool propagate = true) {
            do {
                BoundingBox oldbox = curNode.box;
                ssBVHNode<GO> left = curNode.left;
                ssBVHNode<GO> right = curNode.right;

                // start with the left box
                BoundingBox newBox = left.box;

                // expand any dimension bigger in the right node
                if (right.box.Min.X < newBox.Min.X) {
                    newBox.Min = newBox.Min with { X = right.box.Min.X }; }
                if (right.box.Min.Y < newBox.Min.Y) {
                    newBox.Min = newBox.Min with { Y = right.box.Min.Y }; }
                if (right.box.Min.Z < newBox.Min.Z) {
                    newBox.Min = newBox.Min with { Z = right.box.Min.Z }; }

                if (right.box.Max.X > newBox.Max.X) {
                    newBox.Max = newBox.Max with { X = right.box.Max.X }; }
                if (right.box.Max.Y > newBox.Max.Y) {
                    newBox.Max = newBox.Max with { Y = right.box.Max.Y }; }
                if (right.box.Max.Z > newBox.Max.Z) {
                    newBox.Max = newBox.Max with { Z = right.box.Max.Z }; }
